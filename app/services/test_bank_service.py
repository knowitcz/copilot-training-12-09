...
def dummy_validator(amount: int) -> None:
    pass

def strict_amount_validator(amount: int) -> None:
    if amount > 10000:
...
def test_onlinebankservice_transfer_valid():
    test_double = AccountServiceDouble()
    service = OnlineBankService(test_double, dummy_validator)
    service.transfer_money(1, 2, 300)
    assert test_double.transfers == [(1, 2, 300)]

def test_onlinebankservice_transfer_too_large():
    test_double = AccountServiceDouble()
    service = OnlineBankService(test_double, strict_amount_validator)
    with pytest.raises(ValueError):
